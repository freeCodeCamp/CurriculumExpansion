...
function displayResults(results) {
  ...
  resultsContainer.innerHTML = results.pages
    .map(
      ({ title, id, thumbnail, description }) =>
        `
      <div id="${id}" class="card">
        ...
        <a target="__blank" href=${`https://en.wikipedia.org/wiki/${title}`} class="read-more-link"> 
            Read more
            <svg
...
async function fetchResults(query) {
  try {
    ...
    // TODO: add error message styles and paragraph el
  }
}
