...
// Function to Group newsFeedArray By Sections using reduce.
function groupBySections(arrayInput) {
return arrayInput.reduce((accumulator, currentValue) => {
(accumulator[currentValue.section.name] = accumulator[currentValue.section.name] || []).push(currentValue)
return accumulator
}, {})
};

console.log('Group newsFeedArray By Sections', groupBySections(newsFeedArray))

for(char of newsFeedArray) {
 if(char.likes > 900) {
 }
}

/*
Complete our if condition so that any articles with the likes property are over 900 to create a new property called starred with the value of a boolean set to true. 

We cad append a new article property by simply using dot notation in the context of our for...of loop.

For Example: 
const attendanceSheet = [{students:[{tom:true},{anna:false},{suzie:true}]}]
attendanceSheet.everyone = false
console.log(attendanceSheet)
// returns [{…}, everyone: false]
*/