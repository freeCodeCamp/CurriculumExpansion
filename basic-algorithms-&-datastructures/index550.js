const newsFeed = {
  ...
  articles:  [
    ...
    },
    {
      ...
      id: "e9jbbn",
      image: [],
      date_published: "2019-12-06T02:11:36.000Z",
      date_modified: "",
      author: {
    ...
    },
    {
      ...
      id: "e9jbek",
      image: [],
      date_published: "2019-12-02T13:22:51.000Z",
    ...
    },
    {
      ...
      id: "e9jbdg",
      image: [],
      date_published: "2019-12-01T20:08:30.000Z",
      date_modified: "",
      author: {
        username: "PeoplePerson",
        name: "Pat Comb"
    ...
    },
    {
      ...
      id: "e9jbci",
      image: [],
      date_published: "2019-11-16T00:49:27.000Z",
...
newsFeedArray.filter(filterName => filterName.author.name === 'Julie Holmes')
const searchString = 'Search Results By Author: '
...
if(searchFormula.length > 0) {
  console.log(searchString, searchFormula)
} else {
  console.log(searchString, 'Sorry, but the author name you requested does not match any results.')
}

[1,2,3,4,5].reduce((accum, cv) => accum * cv, 5) // returns 600

function groupBySections(arrayInput) {
return arrayInput.reduce((accumulator, currentValue) => {

}, {})
};

console.log('Group newsFeedArray By Sections', groupBySections())

/*

If we have an array with multiple values that are the same, we can reorganize the array into new arrays of the same values with reduce() and setting the initialValue option to an array.

For Example, given the array: [1,1,4,6,5,4,0,]
[1,1,4,6,5,4,0,].reduce((accumulator, currentValue) => {(accumulator[currentValue] = accumulator[currentValue] || [] ).push(currentValue) 
 return accumulator }, [])

By setting the currentValue argument in an array whenever the currentValue is the same, otherwise create a new array and pushing each current value to the returned accumulator.

Organize the following array of colors: ['red','red','blue','green','yellow','green'] into new arrays with the same colors strings using reduce().
*/