...
colorsArray = ['green', 'yellow', 'blue', 'orange']

/*
The filter() method is a higher order function that calls a provided callback function. The callback function checks the truth value for each element in an array and constructs a new array given 
the result. 

To return a new array that consists only of the string cat from the animalsArray example we would do the following:
animalsArray = ['cat','dog','bird','elephant']
animalsArray.filter(animal => animal === 'cat')
//returns a new array of ["cat"]

Use the filter() method to return only the color green from colorsArray.

Bonus: Construct a new array with filter() from colorsArray including all the colors except green. Look up the ! operator.
