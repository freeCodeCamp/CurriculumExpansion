...
const searchFormula = newsFeedArray.filter(filterName => filterName.author.name === author)
console.log('Search Results By Author: ', searchFormula)
/*
When we type in the name of our author in the author prompt, we now get the search results. 

The results, however, are case sensitive. To make the results case insensitive we can use the toLowerCase() method.

Add the toLowerCase() method to filterd author name in our searchFormula as well as to the author to compare the results. 

For Example:
let namePrompt = prompt('Search a Name') 
const nameArray = ['tom','Julia','tatiana']
nameArray.filter(name => name.toLowerCase() === namePrompt.toLowerCase())
*/