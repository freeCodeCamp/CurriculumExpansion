...
to each item from the `.data()` call. It's creating a rectangle shape for each
population threshold color, which need attributes for the `x` position, the `width`,
the `height`, and the `fill` color.

Chain an `.attr()` method to the rectangle selection to set the `"x"` attribute.
This is a dynamic attribute that changes for each rectangle as D3 iterates over
them - you don't want the rectangles drawn on top of one another. Pass the method
a callback function with parameters for `d` (for data) and `i` (for index). It
should return the linear scale value given the rectangle's index.
*/
