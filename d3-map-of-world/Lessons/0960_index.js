...
you're writing a function that will add a `transform` attribute to the map part
of the visualization. (You only want the pan and zoom behavior to apply to the
countries because you'll be adding a legend and you don't want it to move or scale
as the user pans and zooms.) Next, you'll call the `d3.zoom` function on the SVG
to add the pan/zoom behavior, related event listeners, plus some settings to limit
the user's behavior.

For this lesson, declare the function named `zoomed` with no parameters. In the
body of the function, chain an attribute to the `g` variable - remember, this is
the selection that's holding the `path` elements that create each country outline.
The attribute should be a `"transform"` set to `d3.event.transform` - this will
help D3 store the state of the zoom so it renders the map correctly based on user
interactions.
*/
