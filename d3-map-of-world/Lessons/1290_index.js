...
/*
Finally, you need to tell D3 the values and how to format them. The population
values are available in the `color` scale's domain, and the number formatting
function is saved in your `format` variable.

Chain the `.tickFormat()` method, and pass it a callback function with a parameter
`i` (for index). Return the formatted `color.domain()` selection at the appropriate
index. Since this is not the scale you passed to the `.axisBottom()` call, you
need to manually grab items by their index (hint: use bracket notation). There
is one catch - in order to align the ticks properly between the colors, instead
of grabbing the `ith` population level, you actually need to grab the `ith-1`.
*/
