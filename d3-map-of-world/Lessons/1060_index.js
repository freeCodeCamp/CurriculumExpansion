...
You'll create a rectangle for each one that's then labeled with the corresponding
population threshold.

Using the `const` keyword, declare a variable named `length` and set it to the
length of the range of the `color` scale. (D3 has a `.range()` method that returns
these values for a scale.) Recall that your threshold scale mapped a domain of
population levels onto a range of discrete color values. You want the total number
of these colors, which is the length of the range of that scale.
*/
