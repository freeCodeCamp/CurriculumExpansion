...
`.select()` method, passing it the `this` keyword to create the selection of it.

Remember that all the code chained after the `.enter().append()` sequence is run
for every country - so D3 is drawing a path, styling, and adding these event
handlers to each one. The `this` keyword refers to the `path` element, so the
`.style()` methods you're going to chain in the next lessons will change the
styles of the `path` as the user interacts with that country.
*/
