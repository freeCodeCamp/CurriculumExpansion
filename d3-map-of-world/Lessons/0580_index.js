...
You've already set up a scale to help with this, now you need the code to tell
D3 how to apply the scale to each country's population as D3 draws it on the map.

Chain another `.style()` method, this one for `"fill"`. But this time, instead of
a static value, start a callback function using the parameter `d` (for "data").
Leave the function empty for now, but include curly braces because it will need
multiple lines. You'll start filling in the function next lesson.
*/
