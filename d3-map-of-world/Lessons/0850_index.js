...
`div` that holds your map. You want the tooltip to actually show a bit offset
from where the user's mouse is on the page, so you'll need to apply some styling
to re-position it. You can access where the user's mouse is with the methods
`d3.event.pageX` (for the left-to-right position) and `d3.event.pageY` (for the
up-to-down position).

Chain a `.style()` method to set the `"left"` offset of the tooltip 32 pixels
more than where the mouse is (this will push the tooltip 32 pixels to the right)
- make sure to concatenate `"px"` to this value so D3 knows it's in pixels.
*/
