...
D3 offers the `.on(eventName, eventHandler/callbackFunction)` method for this
purpose, and you'll need to apply three of them for the tooltip to work properly.
There will be one for `"mouseover"` (to change the country's style when the user
hovers over it), one for `"mousemove"` (to display the name and population in a
box that tracks the mouse's movement), and one for `"mouseout"` (to remove the
box and revert the country to its original styling).

One thing to note - two of the three methods will need to use the `this` keyword
in the body of the callback function. That means arrow functions won't work, and
you'll need to use the `function` keyword instead.

To get started, chain an `.on()` method and pass it the event `"mouseover"` and
an empty callback function. The callback needs to use the `function` keyword and
have one parameter named `d`.
*/
