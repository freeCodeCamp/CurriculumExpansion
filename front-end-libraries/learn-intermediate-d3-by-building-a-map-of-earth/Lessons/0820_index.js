...
for the tooltip. Since D3 is dynamically inserting this text into the `div` on
the page, the text can (and should!) have structure to it in the form of HTML
tags. This also makes it easier to style the components.

We created a variable named `dataPoint` and started the HTML string concatenation
to save you some typing. Here's the overall structure for the tooltip text:
...
As you can see, the string needs the data concatenated into it, since that will
change depending on what country the user is moving their mouse over.

You're going to concatenate the name of the country to the end of the current
`dataPoint` string. This is available in the data (`d`) under `properties`, then
`NAME`. Then, concatenate a `<br />` tag.
*/
