...
/*
One final piece of clean-up code and you're done. The `.axisBottom()` function
generated an axis that you then added tick lines to. For the legend, you only
need the tick lines, not the actual horizontal axis line (which would sit along
the colored rectangles). Conveniently, this has a class name of `"domain"` that
you can select and then remove.

Chain two more methods, make sure they come after the closing parentheses of the
`.call()` method. First, select the element with a `"domain"` class name, then
chain the `.remove()` method. Now your legend only shows the tick marks of the
axis.

Congratulations, you've created a functioning, interactive D3 map! These lessons
introduced the basic building blocks for how D3 works, but there are so many more
applications for D3.
