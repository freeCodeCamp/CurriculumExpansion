...
1. The `Planet` class should have an `__init__` method that:
   ...
   - Raises a `ValueError` with the message `Error: Name, planet type, and star must be strings` if any of the arguments passed in is not a string type.
   - Raises a `ValueError` with the message `name, planet_type, and star should be non-empty strings` if any of the arguments passed in is an empty string.
