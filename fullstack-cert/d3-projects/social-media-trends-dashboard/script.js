...
d3.json(
  "https://cdn.freecodecamp.org/curriculum/d3-dashboard-data/data.json",
).then(function (data) {
  globalData = data;
...
function drawDashboard(year, data) {
  ...
  const svgMargin = isMobile ? 60 : 60,
    svgWidth = isMobile ? containerWidth - 40 : containerWidth * 0.7,
  ...
  const pieData = Object.entries(data[index].followers).map(([key, value]) => ({
    key,
    value,
  }));
  const pieColors = d3
